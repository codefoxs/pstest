	...
	*
	...
	if ("`density'"=="" & "`box'"=="" & "`both'"!="") {
        ...
        foreach v in `r(varlist)'{
            local ++i
            local rownames = "`rownames' `v'_U "
            local rownames = "`rownames'`v'_M"
        }
        matrix rownames result_matrix = `rownames'
        matrix colnames result_matrix = Mean_treated Mean_control bias reduct_bias t_test_t t_test_p VT2VC
    }
	if ("`density'"=="" & "`box'"=="" & "`both'"=="") {
	...
	return matrix result_matrix = result_matrix
end
...
syntax varlist(min=1 fv), MWeight(varname) TReated(varname) SUPport(varname) [touse(varname) NOTable DISt LABel GRaph HIST SCatter options(string) RUBin]
	...
	local i 0
	fvrevar `varlist'
    local matrix_row = wordcount("`varlist'") * 2
    matrix result_matrix = J(`matrix_row', 7, .)
	foreach v in `r(varlist)' {
		local ++i 
        ...
        ** Unmatched
        // Mean Treated
        matrix result_matrix[`i'*2-1, 1] = `m1u'
        // Mean Control
        matrix result_matrix[`i'*2-1, 2] = `m0u'
        // %bias
        matrix result_matrix[`i'*2-1, 3] = `bias'
        // %reduct |bias|
        
        // t-test t
        matrix result_matrix[`i'*2-1, 5] = `tbef'
        // t-test p
        matrix result_matrix[`i'*2-1, 6] = `pbef'
        // V(T) / V(C)
        matrix result_matrix[`i'*2-1, 7] = `v_ratiobef'
        
        ** Matched
        // Mean Treated
        matrix result_matrix[`i'*2, 1] = `m1m'
        // Mean Control
        matrix result_matrix[`i'*2, 2] = `m0m'
        // %bias
        matrix result_matrix[`i'*2, 3] = `biasm'
        // %reduct |bias|
        matrix result_matrix[`i'*2, 4] = `absreduc'
        // t-test t
        matrix result_matrix[`i'*2, 5] = `taft'
        // t-test p
        matrix result_matrix[`i'*2, 6] = `paft'
        // V(T) / V(C)
        matrix result_matrix[`i'*2, 7] = `v_ratioaft'
    }
    return matrix result_matrix = result_matrix
...
program define psm2dta
    ...
    local rownames: rowfull result_matrix
    qui gen Variable = "", before(Mean_treated)
    local i = 1
